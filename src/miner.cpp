...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PanamMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PanamMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PanamMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("panam-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PanamMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
